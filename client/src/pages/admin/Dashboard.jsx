...
const Dashboard = () => {
  ...
  useEffect(() => {
    {
      role === "super-admin" && fetchDashboard();
    }
    fetchDashboardAdmin();
  }, []);
      ...
      {role === "super-admin" ? (
        ...
        <div className="flex-1 p-4 md:p-10 bg-blue-50/50">
          ...
          <p className="text-blue-500 font-medium">
            {" "}
            Click the sidebar to add a post!
          </p>
        </div>
      ) : (
        <div className="h-[100%] w-full flex flex-col items-center justify-center">
          ...
          <button
            onClick={() => navigate("/")}
            className="bg-green-500 rounded-sm text-gray-700 font-medium p-4"
          >
            Click to get Admin Access
          </button>
        </div>
      )}
    </>
