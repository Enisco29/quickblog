...
const AddBlog = () => {
  ...
  const generateContentAdmin = async () => {
    if (!title) return toast.error("Please enter a title first");
    try {
      setLoading(true);
      const { data } = await axios.post("/api/blog/generate-admin", {
        prompt: title,
      });
      if (data.success) {
        quillRef.current.root.innerHTML = parse(data.content);
  ...
  const adminSubmit = async (e) => {
    try {
      ...
      const { data } = await axios.post("/api/blog/admin-add", formData, {
        headers: {
          Authorization: `${token}`,
        },
      });
  ...
  const onSubmitHandler = async (e) => {
    try {
      ...
      if (!token || (role !== "super-admin" && role !== "admin")) {
        toast.error("Unathenticated! Please login to continue.");
        return;
      }
  ...
  return (
    <form
      onSubmit={role === "super-admin" ? onSubmitHandler : adminSubmit}
    ...
    >
      <div className="bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded">
        ...
        <select
          ...
          value={category}
          name="category"
          className="nt-2 px-3 py-2 border text-gray-500 border-gray-300 outline-none rounded"
        >
